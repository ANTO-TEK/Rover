...
/**
 * @brief: Reschedule the given task with new WCET and a new function.
 * @pre: The task must be periodic.
 * @pre: The scheduling algorithm must be RMS.
 * @pre: Deadline, phase and period remains the same as when the task was created.
 * 
 * @param pvTaskCode: The new function.
 * @param pcName: Name of the task.
 * @param pvParameters: Parameters to the task function.
 * @param xTaskHandle: Pointer to the task handle.
 * @param xMaxExecTimeTick: Worst-case execution time given in software ticks.
 */

void rescheduleTask(TaskFunction_t pvTaskCode, const char *pcName, void *pvParameters,TaskHandle_t *xTaskHandle, TickType_t xMaxExecTimeTick){
	if(xTaskHandle != NULL){
		...
		if( xTCBArray[ pdTRUE == xIndex].xInUse )
		{
			xTCBArray[ xIndex ].pcName = pcName;
			xTCBArray[ xIndex ].pvTaskCode = pvTaskCode;
			xTCBArray[ xIndex ].xWorkIsDone = pdTRUE;
			xTCBArray[ xIndex ].xExecTime = 0;
			xTCBArray[ xIndex ].xMaxExecTime = xMaxExecTimeTick;
		}
	}
}
